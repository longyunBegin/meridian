...
async function fetchJson(url, { method = 'GET', token = '', body } = {}) {
  ...
  try {
    ...
    let parsed = null
    try { parsed = await res.json() } catch { /* 非 JSON，parsed 保持 null */ }
...
export function createBridge(config) {
  ...
  const macGet = (path) => fetchJson(config.macUrl + path, { token: config.macToken })
  const macPost = (path, body) => fetchJson(config.macUrl + path, { method: 'POST', token: config.macToken, body })
